...
void debug_log(const char* msg) {
	FILE* f = fopen("opencv_debug.log", "a");
	if (f) {
		fprintf(f, "%s\n", msg);
		fclose(f);
	}
...
struct USDX_CvCapture {
	USDX_CvCapture(int index) : cap(index) {
		debug_log("[USDX_CvCapture] Constructor called");
		char buf[128];
		snprintf(buf, sizeof(buf), "[USDX_CvCapture] Opening camera index %d", index);
		debug_log(buf);
	}
	cv::VideoCapture cap;
	UMatWrapper frame;
...
int USDX_cvGetDimSize(UMatWrapper *w, int index)
{
		debug_log("[USDX_cvGetDimSize] called");
		if (!w)
			return 0;
		switch (index) {
		case 0:
...
UMatWrapper *USDX_cvCreateImage(int width, int height, int depth, int channels)
{
		debug_log("[USDX_cvCreateImage] called");
	UMatWrapper *w = NULL;
	try {
...
UMatWrapper *USDX_cvCloneImage(UMatWrapper *src)
{
		debug_log("[USDX_cvCloneImage] called");
	UMatWrapper *w = NULL;
	try {
		w = new UMatWrapper;
...
void USDX_cvReleaseImage(UMatWrapper **w)
{
		debug_log("[USDX_cvReleaseImage] called");
	if (!w)
		return;
	try {
		delete *w;
...
void USDX_cvAbsDiff(UMatWrapper *src1, UMatWrapper *src2, UMatWrapper *dst)
{
		debug_log("[USDX_cvAbsDiff] called");
	if (!src2 || !src2 || !dst)
		return;
	try {
...
void USDX_cvCvtColor(UMatWrapper *src, UMatWrapper *dst, int code)
{
		debug_log("[USDX_cvCvtColor] called");
	if (!src || !dst)
		return;
	try {
...
void USDX_cvEqualizeHist(UMatWrapper *src, UMatWrapper *dst)
{
		debug_log("[USDX_cvEqualizeHist] called");
	if (!src || !dst)
		return;
	try {
...
double USDX_cvThreshold(UMatWrapper *src, UMatWrapper *dst, double threshold, double max_value, int threshold_type)
{
		debug_log("[USDX_cvThreshold] called");
	if (src && dst) {
		try {
...
void USDX_cvSmooth(UMatWrapper *src, UMatWrapper *dst, int smooth_type, int param1, int param2, double param3, double param4)
{
		debug_log("[USDX_cvSmooth] called");
	if (!src || !dst)
		return;
...
void USDX_cvDilate(UMatWrapper *src, UMatWrapper *dst, void *element, int iterations)
{
		debug_log("[USDX_cvDilate] called");
	if (!src || !dst)
		return;
	if (element) {
...
void USDX_cvTransform(UMatWrapper *src, UMatWrapper *dst, cv::Mat *transmat, cv::Mat *shiftvec)
{
		debug_log("[USDX_cvTransform] called");
	if (!src || !dst || !transmat)
		return;
	if (shiftvec) {
...
cv::Mat *USDX_cvCreateMat(int rows, int cols, int type)
{
		debug_log("[USDX_cvCreateMat] called");
	try {
...
void USDX_cvReleaseMat(cv::Mat **mat)
{
		debug_log("[USDX_cvReleaseMat] called");
	if (!mat)
		return;
	try {
		delete *mat;
...
void USDX_cvSetZero(cv::Mat *mat)
{
		debug_log("[USDX_cvSetZero] called");
	if (!mat)
		return;
	*mat = cv::Scalar(0);
}
...
void USDX_cvSetReal2D(cv::Mat *mat, int y, int x, double value)
{
		debug_log("[USDX_cvSetReal2D] called");
	if (!mat)
		return;
	try {
		switch(mat->depth()) {
...
double USDX_cvInvert(cv::Mat *src, cv::Mat *dst, int method)
{
		debug_log("[USDX_cvInvert] called");
	if (!src || !dst)
		return 0;
...
USDX_CvCapture *USDX_cvCreateCameraCapture(int index)
{
		debug_log("[USDX_cvCreateCameraCapture] called");
	try {
...
void USDX_cvReleaseCapture(USDX_CvCapture **capture)
{
		debug_log("[USDX_cvReleaseCapture] called");
	if (!capture)
		return;
	try {
		delete *capture;
...
UMatWrapper* USDX_cvQueryFrame(USDX_CvCapture *capture)
{
		debug_log("[USDX_cvQueryFrame] called");
	if (capture) {
		try {
...
int USDX_cvSetCaptureProperty(USDX_CvCapture* capture, int property_id, double value)
{
		debug_log("[USDX_cvSetCaptureProperty] called");
	if (capture) {
		try {
